...
var topicsWindow = function(){
	...
	var topicsTable = Ti.UI.createTableView({
		top: topTitleBox.top + topTitleBox.height + 10,
		height: 350,
		moveable: true,
		scrollable: false,
		editable: true
	...
	//write a for loop that will pull topics from the array. Declare a variable for the section row and add it to the specific section. Add an event listners to the rows.
	...
	//declare a variable for the done button view
	var doneButton = Ti.UI.createView({
		top: topicsTable.top + topicsTable.height + 30,
		borderColor: "#000",
		borderRadius: 10,
		backgroundColor: "#0084C4",
		height: 50,
	...
	doneButton.add(doneText);
	topicWindow.add(topTitleBox, topicsTable, doneButton);
	topicWindow.open();
