...
NSString * k1508_UtilityApp_Cole_Deonslow$ModuleRequireFormat = @"(function(exports){"
...
@implementation k1508_UtilityApp_Cole_DeonslowObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_k1508_utilityapp_cole_deonslow);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_k1508_utilityapp_cole_deonslow gc];
}
...
{
	// create k1508_UtilityApp_Cole_Deonslow global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "k1508_UtilityApp_Cole_Deonslow" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_k1508_utilityapp_cole_deonslow = [[k1508_UtilityApp_Cole_DeonslowObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_k1508_utilityapp_cole_deonslow];
	
	NSString *_k1508_utilityapp_cole_deonslowNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _k1508_utilityapp_cole_deonslowNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_k1508_utilityapp_cole_deonslow valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:k1508_UtilityApp_Cole_Deonslow$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.k1508_utilityapp_cole_deonslow.kroll" 
