...
//Rare exceptions to indicate a bug in the _k1508_utilityapp_cole_deonslow code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_k1508_utilityapp_cole_deonslow = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_k1508_utilityapp_cole_deonslow]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.k1508_utilityapp_cole_deonslow.module"
