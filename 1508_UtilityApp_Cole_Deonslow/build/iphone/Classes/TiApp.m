...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kk1508_UtilityApp_Cole_DeonslowUserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kk1508_UtilityApp_Cole_DeonslowUserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a k1508_UtilityApp_Cole_Deonslow app using the node.js CLI
